from lib.utility.loading_bar import LoadingBar
import time


class Log:
	def __init__(self, log_each: int, initial_epoch=-1):#, last_steps_state={"loss": 0.0, "accuracy": 0.0, "steps": 0}):
		self.loading_bar = LoadingBar(length=27)
		self.best_accuracy = 0.0
		self.best_loss = float('inf')
		self.log_each = log_each
		self.epoch = -1#initial_epoch #-1以外はerrorになるようだ
		self.initial_epoch = initial_epoch #epoch数の表示バグに対応
	...
	def train(self, len_dataset: int) -> None:
		...
		#self.last_steps_state = last_steps_state#{"loss": 0.0, "accuracy": 0.0, "steps": 0}
	...
	def flush(self) -> None:
		if self.is_train:
			...
			print(
				f"\r┃{(self.initial_epoch+self.epoch):12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃",
#				f"\r┃{self.epoch:12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃",
	...
	def _train_step(self, model, loss, accuracy, learning_rate: float) -> None:
		...
		if self.step % self.log_each == self.log_each - 1:
			...
			print(
				f"\r┃{(self.initial_epoch+self.epoch):12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  ┃{learning_rate:12.3e}  │{self._time():>12}  {self.loading_bar(progress)}",
